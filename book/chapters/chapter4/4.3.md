...
````{figure} images/wave_spectrum
:name: Fig:wavecharacteristics
A wave spectrum is a representation of the combination of waves from different origin occurring at one measuring point. It distinguishes all the different waves in the combination by the frequency $f$ ($=1/T$).}
...
```{figure} images/Wind_speed_Northsea
```
  \caption{Wind speeds in the North Sea. In the logarithmic Gumbel distribution, maximum wins speeds seem to arrive at a maximum of Beaufort 12 >32.7 m/s at a return period of T = 10,000 years (Source KNMI).}
  \label{Fig:Wind_speed_Northsea}
...
```{figure} images/seas_swell_surf
```

\end{subfigure} \hspace{5mm}
\begin{subfigure}[b]{0.45\textwidth}

```{figure} images/changes_to_wavecharacteristics
```
\end{subfigure} 
\caption{Wave characteristics
````