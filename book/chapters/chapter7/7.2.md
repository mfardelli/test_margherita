...
```{figure} images/GroundwaterFlow
```
\\
    \caption{Groundwater flow model for an aquifer under an impermeable dike with leakage through the blankets (adopted from \citet{TRWD2004})}
    \label{Fig:GoundWaterFlow}
...
```{figure} images/USACE_crit_exit_grad_obs
```
\\
  \caption{Severity of seepage as related to upward gradient through top stratum (from {cite}`USACE1956`, Vol. 1, Figure 47)} \label{Fig:USACE_crit_exit_grad_obs}
\end{figure}
...
```
\\
    \caption{Horizontal seepage length ($L$) and head difference ($H$) in Bligh's rule.} 
    \label{Fig:Bligh}
...
```{figure} images/PipingDefinitions
```
\\
    \caption{Definitions for piping assessment models}
    \label{Fig:PipingDefinitions}
...
```{figure} images/Sellmeijer_grain_erosion
```
\\
    \caption[Erosion of sand grains in a piping slit {cite}`Sellmeijer1988`]{Erosion of sand grains in a piping slit {cite}`Sellmeijer1988`. For a river levee, the high water level would be on the right-hand side.}
    \label{Fig:SellmeijerGrainStability}
...
```{figure} images/PipingFaultTree
```
\\
    \caption{Fault tree uplift, heave and piping}\label{Fig:PipingFaultTree}
