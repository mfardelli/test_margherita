...
```{figure} images/figH10_9
:name: Fig:framework_old_new
Framework for derivation of design values and link to safety standards and guidelines.
```
...
(Subsec:Distr_sect)=
## Distribution over sections: accounting for the length effect
...
```{figure} images/figH10_10
:name: Fig:rel_length_relindex
Relation between length of dike trajectory and target reliability index (internal erosion, upper parts of the Rhine)
```
...
```{figure} images/Chapters/chapter_safety_standards/figH10_11new.jpg
:name: Fig:cal_safetyfactor
Calibrated relation between reliability index and safety factor for instability of the inner slope {cite}`Kanning17`. The safety factor $\gamma_n$ in this figure is $\gamma_{stab}$ in the text.
```
...
```{figure} images/Chapters/chapter_safety_standards/ExampleCalibrationCurve.PNG
:name: fig:calibrationpiping
Example calibrated relationship between reliability index and safety factor for different trajectory target reliability index for the failure mode piping {cite}`Teixeira16`
```
