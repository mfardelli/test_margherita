...
```{table} Coefficient values in Equations {eq}`Eq:MeerandBruce2014`, {eq}`Eq:MeerandBruce2014,2`, {eq}`Eq:MeerandBruce2014,red`, and {eq}`Eq:MeerandBruce2014,red2`
:name: Tab:Coef_values_2

| **General Shape** | **Equation {eq}`Eq:MeerandBruce2014` / {eq}`Eq:MeerandBruce2014,red`** | **Equation {eq}`Eq:MeerandBruce2014,2` / {eq}`Eq:MeerandBruce2014,red2`** |
|------------------|--------------------------------|----------------------------------|
| $\frac{q}{\sqrt{g\cdot H^3}} = A(...) \cdot exp - B(...)^{1.3}$ | **A** | **B** | **A** | **B** |
...
```{table} Coefficient values in Equations {eq}`Eq:MeerandBruce2014old` and {eq}`Eq:MeerandBruce2014old,2`
:name: Tab_Coef_values_3

| **General Shape** | **Equation {eq}`Eq:MeerandBruce2014old`** | | **Equation {eq}`Eq:MeerandBruce2014old,2`** | |
|------------------|-----------------|-----------------|-----------------|-----------------|
| $\frac{q}{\sqrt{g\cdot H^3}} = A(...) \cdot \exp\left\lbrace -B(...) \right\rbrace$ | **A** | **B** | **A** | **B** |
