...
```{table}
:name: tab-runup-coefficients

| **General Shape** $\frac{R_u}{H} = A\cdot{\xi}$ <br> and <br> $\frac{R_u}{H} = B - \frac{C}{\sqrt{\xi}}$ | **Equation {eq}`Eq:Simple_Ru`** | **Equation {eq}`Eq:Max_simple_Ru`** |
|:----------------:|:------------------:|:------------------:|
|  | **A** | **B** | **C** |
