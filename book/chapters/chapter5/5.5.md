...
```{figure} images/hillblocks
```
 	\caption{Hillblock (photos courtesy Hillblock b.v.)}
	\label{Fig:Hillblocks}
...
```{figure} images/Definitions_Berm_Reductions
```
 	\caption{Definition Berm Reductions}
	\label{Fig:Berm_red}
...
{numref}`fig:berm_effect` shows the reduction factor $\gamma_B$ for various berm levels and berm widths. The berm width in this figure is expressed as $B_B/H_{m0}$. For a given wave period, this can also be expressed relative to the wave length, so the parameter $B_B/L_0$ can also be used.

\begin{figure} 

```{figure} images/Berm_effect_TAW
```
 	\caption{Berm effect}
	\label{fig:berm_effect}
...
### Composite slopes
Another effect of the berm, or otherwise composite profiles, is that there is no longer a clearly defined single value of the dike slope, and so there is also no clearly defined value of the Iribarren parameter $\xi_{m-1,0}$ . A practical schematization uses the average slope in the zone that is affected by the waves. This zone is defined as the area between 1.5. $H_{m0}$ below SWL and the $z_{2\%}$ run-up level, see {numref}`Fig:schema_av_sl`. But because the $z_{2\%}$ level is not known beforehand, the run-up and overtopping can only be found by an iterative procedure. The width of the berm should not be included in the calculation of the average slope, because its effect is already accounted for separately in the berm reduction factor.

\begin{figure}[htb] 

```{figure} images/Definitions_schematization_average_slope
```
 	\caption{Definitions schematization average slope}
	\label{Fig:schema_av_sl}
...
```{figure} images/muralt_wall_zeeland
```
 	\caption{Muralt wall in Zeeland, (Wikipedia Commons, A. van der Linde)}
	\label{Fig:muralt_wall}
