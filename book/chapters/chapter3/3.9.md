...
This section summarizes the assessments and evaluations of individual, societal and economic risk (see [Deltaprogramma (2014)](#ref-85); [Deltares (2014)](#ref-86); [Ministerie van Infrastructuur en Milieu (2016)](#ref-107)) for further information. These assessments mostly follow the principles introduced in the previous sections of this chapter.
...
(Fig-IR_neth_2)=
<figure>
    <img src="./chapter3_figures/figH3_51aa.jpg" alt="Individual risk for flooding in the Netherlands" style="display: flex; justify-content: center; gap: 10px;">
    <figcaption style="text-align: center; font-size: small;">
        Figure 3.34: Individual risk for flooding in the Netherlands <a href="#Vergouwe2014">[VNK-eindrapportage (2014)](#ref-120)</a>
    </figcaption>
</figure>


The **societal risk** has been considered at a national level. The proposed limit lines according to the method of [TAW (1985)](#ref-77); see also [section 3.8.3](subsec-societal-risk) for background) are shown in [Figure 3.35](Fig-Comparison_fncurves). These are considered for different values of a so-called policy factor $\beta$. This factor relates to voluntariness of exposure and the benefits of the risk-bearing activities. For flood risks typical values between $\beta$ = 0.1 and 1.0 have been proposed in earlier work. Similar to industrial safety a risk averse limit line is used ($\alpha$ = 2), which means that accidents with 10 times as many fatalities are accepted with a 100 times smaller probability.

(Fig-Comparison_fncurves-temp)=
The figure shows that the societal risk in the current baseline situation (“reference situation” - black line) is larger than typically acceptable levels according to the TAW model. When different sets of more stringent safety standards for the dike rings are applied (for example safety standards based on individual and economic risk - see the red and blue lines below) the actual risks decrease below the value of the limit line for $\beta$ = 1. The highest societal risks are found for dike rings in the Southwest of the country (e.g. Alblasserwaard) and along the Waal River (e.g. the Betuwe) - see [Deltares (2014)](#ref-86) for further details. For six areas with relatively high societal risk levels, more stringent safety standards have been proposed based on this perspective.
...
<figure>
    ...
    <figcaption style="text-align: center; font-size: small;">
        Figure 3.35: Comparison of nationwide FN curves for flooding with potential limit lines with varying Beta values according to the TAW model <a href="#Deltaprogramma2014">[Deltaprogramma (2014)](#ref-85)</a>
    </figcaption>
</figure>
...
Due to the implementation of these safety standards the individual risks behind flood defences will be smaller than 10 to 5 per year. Also societal risks will be significantly reduced: the probability of an event with more than 100 fatalities will decrease with a factor 15 to 20. Finally, economic risks will be reduced by a factor 25 [Ministerie van Infrastructuur en Milieu (2016)](#ref-107).

(Fig-Proposed_new_SS)=
<figure>
    <img src="./chapter3_figures/figH3_342.jpg" alt="Proposed safety standards, maximal acceptable failure probabilities for dike trajectories"  style="display: flex; justify-content: center; gap: 10px;">
    <figcaption style="text-align: center; font-size: small;">
        Figure 3.36: Proposed safety standards, maximal acceptable failure probabilities for dike trajectories <a href="#MinisterieI&M2016">[Ministerie van Infrastructuur en Milieu (2016)](#ref-107)</a>.
    </figcaption>
</figure>

::: {admonition} **Textbox 3.9: Deriving safety standards based on individual risk**
:class: tip
...
The actual individual risk in an area (\(IR\)) should be smaller than the acceptable individual risk determined by the government (\(IR_{acc}\)). This implies:  
\[
IR \leq IR_{acc}
\]  
...
\]  
In which:  
- \(P_{f}\): the probability of flooding \([\text{per year}]\)  
- \(F_{D}\): the mortality in the area \([-]\)  
- \(F_{E}\): the evacuation fraction \([-]\)  

By re-arranging both formulas, we find that:  
\[
P_{f,req} \leq \frac{IR_{acc}}{F_{D} \cdot (1 - F_{E})}
\]  
...
|                             | **Shallow Polder**      | **Deep Polder**          |
|-----------------------------|-------------------------|--------------------------|
| **Evacuation fraction (\(F_{E}\))** | 0                       | 0.9                      | 0                        | 0.9                      |
| **Mortality (\(F_{D}\))**          | 0.01                    | 0.01                     | 0.1                      | 0.1                      |
| **Resulting failure probability or safety standard** | 0.001                  | 0.0001                   | 0.01                    | 0.001                    |

**Comparison between new and old safety standards**  

A brief comparison between the old and new safety standards is made in the table below and further discussed.

|                            | **New standards**                                          | **Old standards**                                         |
|----------------------------|-----------------------------------------------------------|----------------------------------------------------------|
| **Basis and derivation**   | Assessments of individual, societal risk, and cost-benefit analysis for the entire country | Cost-benefit analysis/economic optimization for South Holland by van Dantzig |
