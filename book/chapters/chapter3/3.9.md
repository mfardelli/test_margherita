...
```{figure} images/figH3_51aa
```
 	\caption{Individual risk for flooding in the Netherlands {cite}`Vergouwe2014`}
 	\label{Fig:IR_neth_2}

\end{figure}

The **societal risk** has been considered at a national level. The proposed limit lines according to the method of \citeauthor{TAW1985} (\citeyear{TAW1985}; see also {numref}`Subsec:SR` for background) are shown in {numref}`Fig:Comparison_fncurves`. These are considered for different values of a so-called policy factor $\beta$. This factor relates to voluntariness of exposure and the benefits of the risk-bearing activities. For flood risks typical values between $\beta$ = 0.1 and 1.0 have been proposed in earlier work. Similar to industrial safety a risk averse limit line is used ($\alpha$ = 2), which means that accidents with 10 times as many fatalities are accepted with a 100 times smaller probability.
...
```{figure} images/figH3_342
```
 	\caption{Proposed safety standards, maximal acceptable failure probabilities for dike trajectories {cite}`MinisterieI&M2016`.}
 	\label{Fig:Proposed_new_SS}

\end{figure}

\fcolorbox {black}{white}{
\begin{minipage}{1.0\textwidth}
**Textbox 3.9: Deriving safety standards based on individual risk** 
...
In which:
- $P_{f}$ is the probability of flooding $[/year]$
- $F_{D}$ is the mortality in the area $[-]$
- $F_{E}$ is the evacuation fraction $[-]$

By re-arranging both formulas we find that $P_{f,req} \leq IR_{acc}/(F_{D}\cdot{(1 - F_{E})})$.

Where $P_{f,req}$ is the required failure probability for an area (to comply to the individual risk limit).

This expression is applied to a number of typical cases for the Netherlands. We distinguish between areas with high or low evacuation fractions, and areas with relatively high or low mortality (deep vs. less deep polders). As an acceptable value for the individual risk $IR_{acc}$=10 to 5 per year is chosen. The results in the latest rows show the required failure probability values (or safety standards) that would result. Typical values range from 10 to 2 per year for shallow polders that can be well evacuated, to 10 to 4 per year for deep polders with limited possibilities for evacuation.

\begin{table}[H]
\centering

\label{tab:comparison_new_and_old_standards}
\begin{tabular}[width=0.7\textwidth]{|l|l|l|l|l|}
\hline
                                                 & \multicolumn{2}{l|}{Shallow Polder} & \multicolumn{2}{l|}{Deep Polder} \\ \hline
Evacuation fraction $F_{E}$                      & 0                & 0.9              & 0               & 0.9            \\ \hline
Mortality $F_{D}$                                & 0.01             & 0.01             & 0.1             & 0.1            \\ \hline
Resulting failure probability or safety standard & 0.001               & 0.0001                & 0.01               & 0.001              \\ \hline
\end{tabular}
\end{table}

**Comparison between new and old safety standards**\\
A brief comparison between the old and new safety standards is made in table  and further discussed below.

```{table} 
| | New standards                                                                             | Old standards                                                                                                                                                           \\ \hline|
| :--- | :--- | :--- |
| Basis and derivation | Assessments of individual,|
| societal risk\\ | and cost benefit analysis|
| for the entire country | Cost benefit analysis /\\ | | economic optimization for \\ | | South Holland by van Dantzig \\ | ||
```

When compared to the old system of safety standards, a first major change is that these standards refer to the failure probability of a dike system, whereas the old standards focused on the probability of exceedance of a design water levels or loads . The use of the failure probability will enable a more complete assessment of the different failure mechanisms of a flood defence. The new safety standards will also take into account more directly the contribution of various dike sections and structures in a dike ring to the failure probability of the system and the so-called length effect. 
\end{minipage} }

\fcolorbox {black}{white}{
\begin{minipage}{1.0\textwidth}
The second change concerns the spatial unit to which the safety standards apply. The new safety standards are valid for a so-called trajectory, which can be a part of a dike ring. The length of trajectories ranges from several kilometer to more than 30 km. A demarcation between trajectories is often made when there are differences in the consequences of breaching. Whereas the old standards were the same of the entire dike ring, the new system does include different standards for different parts of a dike ring if the consequences differ. 
...
(Subsec:app_impl)=
### Application and implementation of new safety standards
