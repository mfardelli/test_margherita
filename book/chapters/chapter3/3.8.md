...
```{figure} images/figH3_new
```
 	\caption{A fictitious calculated FN curve (lower left) and a limit line (diagonal).}
 	\label{Fig:Fic_fncurve}
...
{numref}`Fig:eco_opt` shows the costs and risks as a function of the accepted failure probability of a system. The economic optimum is found when the total costs are minimal. For this situation the following is valid:
...
```{figure} images/figH3_33
```
 	\caption{Economic optimization, costs, risks and total costs as a function of the failure probability of the system}
 	\label{Fig:eco_opt2}
